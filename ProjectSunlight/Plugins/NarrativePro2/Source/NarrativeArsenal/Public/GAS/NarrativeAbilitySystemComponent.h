...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FOnDamagedBy, UNarrativeAbilitySystemComponent*, DamagerCauserASC, const float, Damage);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FOnDealtDamage, UNarrativeAbilitySystemComponent*, DamagedASC, const float, Damage);
...
public:
	...
	virtual void DamagedBy(UNarrativeAbilitySystemComponent* DamageCauser, const float Damage);

	//Workaround for attribute changed GEData not containing a valid instigator - we need the instigator so bots know when they receive damage. 
	virtual void DealtDamage(UNarrativeAbilitySystemComponent* DamagedTarget, const float Damage);
