...
struct NARRATIVEARSENAL_API FDialoguePlayParams
{
	...
	//Any node without a speaker defined will have this NPC used as the speaker instead. Useful for having many different characters reuse a generic dialogue asset
	UPROPERTY(EditDefaultsOnly, BlueprintReadWrite, Category = "Play Params")
	TArray<class ANarrativeNPCCharacter*> Speakers;

	//Non-Unique NPCs can play dialogue, however you need to specify which NPC wants to play the dialogue in here, so we know which NPC you need in the dialogue
	//TODO wont work in networked games as playparams aren't passed to clients yet 
	UPROPERTY(EditDefaultsOnly, BlueprintReadWrite, Category = "Play Params")
	TArray<class ANarrativeNPCCharacter*> NonUniqueNPCs;
};
