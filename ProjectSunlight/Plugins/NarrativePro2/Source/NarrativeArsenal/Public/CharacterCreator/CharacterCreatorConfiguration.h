...
public:
	...
	//The transform in the level the camera should lerp to when this page is selected 
	UPROPERTY(BlueprintReadWrite, EditDefaultsOnly, Category = "Attribute")
	FTransform PageCameraTransform;
...
public:
	...
	//The visual class that should be spawned if we're using this form 
	UPROPERTY(BlueprintReadWrite, EditDefaultsOnly, Category = "Form")
	TSubclassOf<class ANarrativeCharacterVisual> CharacterVisualClass;

	//The base mesh this form should have by default - usually hidden
	UPROPERTY(BlueprintReadWrite, EditDefaultsOnly, Category = "Form")
	TObjectPtr<class USkeletalMesh> BaseMesh;

	//Whether or not to hide the base mesh. Usually you'd hide it and your visible body parts would follow this.  
	UPROPERTY(BlueprintReadWrite, EditDefaultsOnly, Category = "Form")
	bool bHideBaseMesh;
