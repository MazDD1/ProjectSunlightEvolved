	...
	for (auto& Speaker : Speakers)
	{
		if (SpeakerAvatars.Contains(Speaker.GetSpeakerID()))
		{
			if (ANarrativeCharacter* NChar = Cast<ANarrativeCharacter>(SpeakerAvatars[Speaker.GetSpeakerID()]))
			{
				...
				{
					ASC->AddLooseGameplayTags(Speaker.OwnedTags);
				}
			}
		}
	}
	...
	for (auto& Speaker : Speakers)
	{
		if (SpeakerAvatars.Contains(Speaker.GetSpeakerID()))
		{
			if (ANarrativeCharacter* NChar = Cast<ANarrativeCharacter>(SpeakerAvatars[Speaker.GetSpeakerID()]))
			{
				...
				if (UAbilitySystemComponent* ASC = NChar->GetAbilitySystemComponent())
				{
					ASC->RemoveLooseGameplayTags(Speaker.OwnedTags);
				}
			}
		}
	}
...
AActor* UDialogue::LinkSpeakerAvatar_Implementation(const FSpeakerInfo& Info, const int32 Idx)
{
	...
	if (!Info.GetSpeakerID().IsNone())
	{
		...
		{
			//We have a non-unique speaker so can't find it via the NPC subsystem, we instead grab it from the PlayParams. 
			if (Info.NPCDataAsset->bAllowMultipleInstances)
			{
				for (auto& NonUniqueNPC : PlayParams.NonUniqueNPCs)
				{
					if (UNPCDefinition* NPCDef = NonUniqueNPC->GetNPCDefinition())
					{
						if (NPCDef == Info.NPCDataAsset)
						{
							return NonUniqueNPC;
						}
					}
		...
		else if (!SpeakerAvatars.Contains(Info.GetSpeakerID()) && IsValid(Info.SpeakerAvatarClass))
		{
