...
void UEquippableItem_Clothing::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (!ClothingMeshData.MeshMaterials.Num() && ClothingMeshData.Mesh)
	{
...
void UEquippableItem_Clothing::ApplyClothingMesh()
{
	if (ANarrativeCharacter* CharacterOwner = Cast<ANarrativeCharacter>(GetOwningPawn()))
	{
		...
		{
			CharVisual->SetMeshAppearance(EquippableSlot, ClothingMeshData);
...
void UEquippableItem_Clothing::HandleEquip_Implementation()
{
	if (ANarrativeCharacter* CharacterOwner = Cast<ANarrativeCharacter>(GetOwningPawn()))
	{
		...
		checkf(IsValid(CharVisual), TEXT("Tried equipping clothing before the CharVisual was initialized! This shouldn't happen."));
...
void UEquippableItem_Clothing::HandleUnequip_Implementation()
{
	...
	{	
		if (ANarrativeCharacterVisual* CharVisual = CharacterOwner->GetCharacterVisual())
		{
			CharVisual->HandleUnEquipClothing(this);
		}
	}
}
