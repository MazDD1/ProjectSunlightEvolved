...
float UNarrativeCharacterMovement::VisualizeMovement() const
{
	...
	//Forward direction 
	{
		...
		const FVector DebugLocation = GetActorLocation();//TopOfCapsule + HeightOffset * -GetGravityDirection();
		DrawDebugDirectionalArrow(GetWorld(), DebugLocation, DebugLocation + FwdVect * 100.f,
		...
		FString DebugText = FString::Printf(TEXT("Forward Vector: %s"), *FwdVect.ToCompactString());
...
{
	return Super::CanAttemptJump() && !IsMovementLocked();
}
...
{
	if (MirrorMovement && !IsClimbing())
	{
		bWantsToCrouch = MirrorMovement->IsCrouching();
	}
...
bool UNarrativeCharacterMovement::IsMovementLocked() const
{
	...
	return NarrativeCharacterOwner->HasMatchingGameplayTag(FNarrativeGameplayTags::Get().State_Movement_Lock);
}
...
void UNarrativeCharacterMovement::UpdateClimbStateBeforeMovement()
{
	...
	if (IsTransitionClimbMode() && CanClimbTransitionFinish())
	{
		...
		//Mantling wants to leave climbing 
		if (ClimbMode == ENarrativeClimbMode::Mantling)
		{
...
ENarrativeClimbMode UNarrativeCharacterMovement::ComputeDesiredClimbMode() const
{
	...
	if (FMath::Abs(LocalInputVector.Y) > 1e-3f || !bCanExitClimbStrafe)
	{
		NewDesiredClimbMode = ENarrativeClimbMode::Strafe;
	}
	else if (!LocalInputVector.IsNearlyZero())
	{
...
{
	if (ClimbMode == ENarrativeClimbMode::JumpWarping || ClimbMode == ENarrativeClimbMode::Mantling)
	{
		return !bIsWarping;
	}
	else
...
void UNarrativeCharacterMovement::HandleClimbJump()
{
	...
	if (FMath::Abs(JumpInput.Y) > .1f)
	{
		...
		if (PossibleTurnSurface.IsSet())
		{
			...
			Velocity += PreviousClimbSurface.TransformVector(FVector(-100.f, 0.f, TurnJumpZVelocity));
	...
	if (!IsTransitionClimbMode())
	{
		...
		if (PossibleHopSurface.IsSet())
		{
			...
			ClimbMode = ENarrativeClimbMode::JumpWarping;
			//FVector LocalBoost = JumpInput.X < -1e-3f ? ClimbLocalDecendTransitionBoost : ClimbLocalAscendTransitionBoost;   

			//Dont apply any boosts, we're letting montage deal with hops 
			//Velocity += ClimbSurface.TransformVector(LocalBoost);


			//We want motion warping to control hopping to the target, so we'll set warp target now. Transition should still exit as usual. 
			if (ANarrativeCharacter* NCharOwner = GetNarrativeCharacterOwner())
			{
				UAnimMontage* JumpMontage = HopUpMontage;

				if (FMath::Abs(JumpInput.Y) > 0.1f)
				{
					JumpMontage = JumpInput.Y < 0.f ? HopLeftMontage : HopRightMontage;
				}

				if (FMath::Abs(JumpInput.X) > 0.1f)
				{
					JumpMontage = JumpInput.X < 0.f ? HopDownMontage : HopUpMontage;
				}

				NCharOwner->PlayAnimMontage(JumpMontage);

				if (UMotionWarpingComponent* WarpComp = NCharOwner->GetMotionWarpingComponent())
				{
					
					FRotator Rotation = GetClimbCapsuleTargetTransform(PossibleHopSurface.GetValue()).GetRotation().Rotator();
					FVector Location = GetClimbCapsuleTargetTransform(PossibleHopSurface.GetValue()).GetLocation();

					Location.Z -= CapHH();

					WarpComp->AddOrUpdateWarpTargetFromLocationAndRotation("ClimbWarp", Location, Rotation);

					bIsWarping = true; 

					GetWorld()->GetTimerManager().SetTimer(TH_IsWarping, this, &UNarrativeCharacterMovement::RemoveClimbWarpingMode, JumpMontage->GetPlayLength(), false);
				}
			}
	...
	if (!IsTransitionClimbMode())
	{
		...
		{
			TOptional<FTransform> MantleTrans = TryFindMantleSurface(JumpInput);

			if (MantleTrans.IsSet() && IsValid(ClimbMantleMontage))
			{
				ClimbMode = ENarrativeClimbMode::Mantling;
				//FVector LocalBoost = JumpInput.X < -1e-3f ? ClimbLocalDecendTransitionBoost : ClimbLocalAscendTransitionBoost;   
				...
				if (ANarrativeCharacter* NCharOwner = GetNarrativeCharacterOwner())
				{
					NCharOwner->PlayAnimMontage(ClimbMantleMontage);

					if (UMotionWarpingComponent* WarpComp = NCharOwner->GetMotionWarpingComponent())
					{

						FRotator Rotation = MantleTrans.GetValue().GetRotation().Rotator();
						FVector Location = MantleTrans.GetValue().GetLocation();

						WarpComp->AddOrUpdateWarpTargetFromLocationAndRotation("ClimbWarp", Location, Rotation);

						bIsWarping = true;

						GetWorld()->GetTimerManager().SetTimer(TH_IsWarping, this, &UNarrativeCharacterMovement::RemoveClimbWarpingModeEndClimb, 1.72f, false);
...
TOptional<FTransform> UNarrativeCharacterMovement::TryFindMantleSurface(FVector2D InputDirection) const
{
	FTransform WorldT = ClimbSurface;
	WorldT.SetRotation(FRotator(0.f, WorldT.Rotator().Yaw, 0.f).Quaternion());


	const FVector WarpTo = WorldT.GetRotation().GetForwardVector() * (CapR()) + WorldT.GetLocation();


	return FTransform(GetForwardVector().Rotation().Quaternion(), WarpTo);
	//TODO trace from top of capsule to bottom to ensure space, then do forward check to ensure no walls or anything blocking us from climbing up 
	//FHitResult Hit;

	//Perform a trace from top of mantle to bottom to ensure no world geometry will block the place we're trying to reach
...
TOptional<FTransform> UNarrativeCharacterMovement::TryFindClimbTurnSurface(FVector2D InputDirection) const
{
	...
	const FVector ConvexStart = UpdatedComponent->GetComponentTransform().TransformPosition(FVector(-ClimbTurnReachDepth, 0.f, ZOffset));
	...
	const float MaxReach = ClimbJumpReachDistance;
	const float MinReach = 20.f + (InputDirection.X < .1f) * 20.f + (FMath::Abs(InputDirection.Y) > .1f) * 20.f;
	UE_LOG(LogTemp, Warning, TEXT("reach %f"), MinReach);
	auto CollisionShape = FCollisionShape::MakeCapsule(10.f, (MaxReach - MinReach) / 2.f); // UpdatedPrimitive->GetCollisionShape()

	TOptional<FTransform> PossibleConvexSurface = TryFindClimbSurface(FTransform(ConvexRotation,ConvexStart), CollisionShape, 150.f, false);
	...
	FVector ShallowStart = UpdatedComponent->GetComponentTransform().TransformPosition(FVector(XOffset + ClimbTurnReachDepth, TurnDirection * ConcaveOffset, ZOffset));
	if (CVarClimbDebug.GetValueOnGameThread())
	{
	...
	TOptional<FTransform> ShallowAttempt = TryFindClimbSurface(FTransform(ConcaveRotation, ShallowStart), CollisionShape, ConcaveOffset + 50.f, false);
	if (ShallowAttempt.IsSet()) return ShallowAttempt;

	FVector DeepStart = UpdatedComponent->GetComponentTransform().TransformPosition(FVector(XOffset + ClimbTurnReachDepth * 1.5f, TurnDirection * ConcaveOffset, ZOffset));
	TOptional<FTransform> DeepAttempt = TryFindClimbSurface(FTransform(ConcaveRotation, DeepStart), CollisionShape, ConcaveOffset + 50.f, false);
...
void UNarrativeCharacterMovement::PhysClimb(float deltaTime, int32 Iterations)
{
	...
	if (!IsTransitionClimbMode())
	{
		if(ClimbMode == ENarrativeClimbMode::Strafe && bCanExitClimbStrafe)
		{
			ClimbMode = ENarrativeClimbMode::Idle;
			
		}
		
		if(FMath::Abs(LocalInputVector.Y) > 0.1 && bCanExitClimbStrafe)
		{
			LastLocalYInput = LocalInputVector.Y;
		}

		if (FMath::Abs(LocalInputVector.Y) > .1f || !bCanExitClimbStrafe) // Wants strafe 
		{
			float Strafe = LocalInputVector.Y * GetMaxSpeed() * deltaTime;

			if (!bCanExitClimbStrafe)
			{
				Strafe = LastLocalYInput * GetMaxSpeed() * deltaTime;
			}
			...
			{
				TOptional<FTransform> PossibleTurnSurface = TryFindClimbTurnSurface(FVector2D(0.f, Strafe));
				if (PossibleTurnSurface.IsSet())
				{
					SetClimbSurface(PossibleTurnSurface.GetValue());
					ClimbMode = ENarrativeClimbMode::Turn;
	...
	while(remainingTime >= MIN_TICK_TIME && Iterations < MaxSimulationIterations)
	{
		...
		//Jumping and mantling are now handled via motion warping - tbh almost entire climb system could just be warping 
		if (ClimbMode != ENarrativeClimbMode::JumpWarping && ClimbMode != ENarrativeClimbMode::Mantling)
		{
		...
		{
			//UE_LOG(LogTemp, Warning, TEXT("climb mode was jump, need to let root motion carry our Z RMV: %s"), *AnimRootMotionVelocity.ToString());
		}
...
FTransform UNarrativeCharacterMovement::GetWarpClimbCapsuleTargetTransform(FTransform InClimbSurface) const
{
	float XOffset = CapR() + ClimbWallOffset;
	float ZOffset = ClimbHangOffset;
	FVector SurfaceNormal = -InClimbSurface.GetUnitAxis(EAxis::X);
	FVector TargetLocation = InClimbSurface.GetLocation() + SurfaceNormal * XOffset - UpdatedComponent->GetUpVector() * ZOffset;
	FQuat TargetRotation = FRotationMatrix::MakeFromXZ(InClimbSurface.GetRotation().GetForwardVector(), FVector::UpVector).ToQuat();
	return FTransform(TargetRotation, TargetLocation);
...
void UNarrativeCharacterMovement::RemoveClimbWarpingMode()
{
	bIsWarping = false;
...
void UNarrativeCharacterMovement::RemoveClimbWarpingModeEndClimb()
{
	bIsWarping = false;
