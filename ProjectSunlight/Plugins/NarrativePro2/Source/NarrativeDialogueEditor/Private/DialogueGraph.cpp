...
bool UDialogueGraph::Modify(bool bAlwaysMarkDirty/*=true*/)
{
	...
	{
		if (DialogueAsset->DialogueTemplate)
		{
			DialogueAsset->DialogueTemplate->Modify(true);
		}
	}
		...
		if (Pin->Direction == EEdGraphPinDirection::EGPD_Output)
		{
			if (UDialogueGraphNode* WiredFrom = Cast<UDialogueGraphNode>(Node))
			{
				if (WiredFrom->DialogueNode)
				{
					...
					const FScopedTransaction Transaction(NSLOCTEXT("RewireTransact", "TrasactKey", "Dialogue wire changed"));
					WiredFrom->DialogueNode->Modify();
					//Rebuild replies
...
class UDialogueNode* UDialogueGraph::DuplicateNode(UDialogueNode* OldNode)
{
	if (IsValid(OldNode))
	{
		if (UDialogueBlueprint* DialogueAsset = Cast<UDialogueBlueprint>(GetOuter()))
		{
			if (DialogueAsset->DialogueTemplate)
			{
				UDialogueNode* DupNode = Cast<UDialogueNode>(StaticDuplicateObject(OldNode, OldNode->GetOuter()));

				if (UDialogueNode_NPC* NPCNode = Cast<UDialogueNode_NPC>(DupNode))
				{
					DialogueAsset->DialogueTemplate->NPCReplies.AddUnique(NPCNode);
				}
				else if (UDialogueNode_Player* PlayerNode = Cast<UDialogueNode_Player>(DupNode))
				{
					DupNode->OwningDialogue->PlayerReplies.AddUnique(PlayerNode);
				}
